...
import { crudGetOne as crudGetOneAction, crudRetrieve as crudRetrieveAction } from '../../actions/dataActions';
...
class Retrieve extends Component {
    ...
    handleSubmit(event) {
        ...
        this.props.crudRetrieve(this.props.resource, this.props.id, this.props.data, this.getBasePath());
    }
    ...
    render() {
        ...
        const defaultTitle = translate('hcore.page.retrieve', {
            name: `${resourceName}`,
            id,
            data,
        });
            ...
            <div>
                <Card style={{ opacity: isLoading ? 0.8 : 1 }}>
                    ...
                    <form onSubmit={this.handleSubmit}>
                        <CardText>{translate('hcore.message.are_you_sure_retrieve')}</CardText>
                        <Toolbar style={styles.toolbar}>
                            <ToolbarGroup>
                                <RaisedButton
                                    primary
                                    type="submit"
                                    label={translate('hcore.action.retrieve')}
                                    icon={<ActionCheck />}
                                    style={styles.button}
                                />
                                <RaisedButton
                                    label={translate('hcore.action.cancel')}
                                    icon={<AlertError />}
                                    onClick={this.goBack}
                                    style={styles.button}
                                />
...
// eslint-disable-next-line react/no-unused-prop-types
    ...
    crudRetrieve: PropTypes.func.isRequired,
...
const enhance = compose(
    connect(
        mapStateToProps,
        { crudGetOne: crudGetOneAction, crudRetrieve: crudRetrieveAction },
    ),
    translate,
);

export default enhance(Retrieve);
