...
import DefaultRowStyle from './RowStyle';
...
class Datagrid extends Component {
    ...
    render() {
        const { resource, children, ids, isLoading, data, currentSort, basePath, styles = defaultStyles, muiTheme, rowStyle = DefaultRowStyle, options, headerOptions, bodyOptions, rowOptions, width } = this.props;
        return (
                    ...
                    <TableRow style={muiTheme.tableRow}>
                        {
                            React.Children.map(children, (field, index) => (
                                width === 1 && !field.props.smallScreen ? undefined :
                                    <DatagridHeaderCell
                ...
                <DatagridBody resource={resource} ids={ids} data={data} basePath={basePath} styles={styles} rowStyle={rowStyle} isLoading={isLoading} options={bodyOptions} rowOptions={rowOptions}>
                    {
                        React.Children.map(children, field => (
                            width === 1 && !field.props.smallScreen ? undefined : field
                        ))
                    }
                </DatagridBody>
            </Table>
