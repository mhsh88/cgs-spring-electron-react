 ...
 */
export class SelectArrayInputComponent extends Component {
    state = {
        values: [],
    };
    ...
    getChoicesForValues = (values, choices = []) => {
        ...
        return values
            .map(value => choices.find(c => c[optionValue] === value[optionValue]) || { [optionValue]: value[optionValue], [optionText]: value[optionValue] })
    ...
    extractIds = (eventOrValue) => {
        ...
        if (Array.isArray(value)) {
            return value.map(o => ({ [this.props.optionValue]: o.value }));
            // return value.map(o => o.value);
        }
        return [{ [this.props.optionValue]: value }];
        // return [value];
    };
    ...
    formatChoice = (choice) => {
        const { language, optionText, optionValue, translateChoice, translate } = this.props;
        /* let choiceText = React.isValidElement(optionText) ?
        ...
        /* if (typeof choiceText === 'object') {
            const hasResourceAnyLocale = Object.keys(choiceText).reduce((result, key) => {
                if (key !== INITIAL_LANGUAGE) result.push(key); return result;
            }, []).length > 0;
            if (hasResourceAnyLocale) {
                choiceText = get(choiceText, language);
                if (choiceText === undefined || choiceText === null) {
                    choiceText = get(choiceText, DEFAULT_LANGUAGE);
                }
                if (choiceText === undefined || choiceText === null) {
                    choiceText = get(choiceText, get(choiceText, INITIAL_LANGUAGE));
                }
            }
        }*/
        return {
...
SelectArrayInputComponent.propTypes = {
...
SelectArrayInputComponent.defaultProps = {
    addField: true,
    choices: [],
    onBlur: () => true,
    onChange: () => true,
...
const SelectArrayInput = enhance(SelectArrayInputComponent);
