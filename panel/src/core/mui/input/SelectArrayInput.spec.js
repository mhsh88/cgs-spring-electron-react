...
import { SelectArrayInputComponent } from './SelectArrayInput';
...
describe('<SelectArrayInput />', () => {
    ...
    it('should use a ChipInput', () => {
        const wrapper = shallow(<SelectArrayInputComponent {...defaultProps} />);
        const ChipInputElement = wrapper.find('ChipInput');
        assert.equal(ChipInputElement.length, 1);
    });

    it('should use the input parameter value as the initial input value', () => {
        const wrapper = shallow(<SelectArrayInputComponent {...defaultProps} input={{ value: [1, 2] }} />);
    ...
    it('should pass choices to the ChipInput as dataSource', () => {
        const wrapper = shallow(<SelectArrayInputComponent
            {...defaultProps}
            choices={[
                { id: 1, name: 'Book' },
                { id: 2, name: 'Video' },
    ...
    describe('error message', () => {
        it('should not be displayed if field is pristine', () => {
            const wrapper = shallow(<SelectArrayInputComponent {...defaultProps} meta={{ touched: false }} />);
            const ChipInputElement = wrapper.find('ChipInput');
            assert.equal(ChipInputElement.prop('errorText'), false);
        });

        it('should not be displayed if field has been touched but is valid', () => {
            const wrapper = shallow(<SelectArrayInputComponent {...defaultProps} meta={{ touched: true, error: false }} />);
            const ChipInputElement = wrapper.find('ChipInput');
            assert.equal(ChipInputElement.prop('errorText'), false);
        });

        it('should be displayed if field has been touched and is invalid', () => {
            const wrapper = shallow(<SelectArrayInputComponent {...defaultProps} meta={{ touched: true, error: 'Required field.' }} />);
            const ChipInputElement = wrapper.find('ChipInput');
